...
stage = Surface((600, 480), SRCALPHA)
you_are_dead = pygame.image.load('death.png')
stage.blit(you_are_dead, (0, 0))
you_are_dead = stage
...
def main():
    ...
    pygame.display.set_caption("The Legend of N++")
    ...
    elif leveltwo and not levelone:
        level = [
            "PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP",
            "P                         P                        P        P",
            "P                         P                        N        P",
            "P                                     II           PIIIIIIEIP",
            "P             IIII        PIIIIIIIIII         PIIIIP",
            "P                                          PIIP",
            "P                                          P",
            "P                                          P",
            "P                    PIIIIIIIIII           P",
            ...
            "P                                          P",
            "P                                          P",
            "PGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGP",
            "PHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHP",
            "PWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWP", ]
...
class Player(Entity):
    ...
    def animate(self):
        ...
        if state[0]:
            if not self.moving and not self.attacking and self.standcounter >= 30:
                if state[2]:
...
class Enemy(Entity):
    ...
    def take_damage(self):
        if self.target.attacking and self.target.attackcounter >= 10:
...
class PlatformEnemy(Entity):
    def __init__(self, x, y, right, left, target, platstart, platend):
        Entity.__init__(self)
        self.pls = platstart
        self.ple = platend
        self.xvel = 0
    ...
    def update(self, left, right, platforms):
        ...
        if self.rect.left+16 in range(self.pls, self.ple):
            ...
            elif self.rect.left-16 in range(self.target.rect.left-286, self.rect.right+286) and self.onGround:
                if self.target.rect.left >= self.rect.left:
                    self.xvel = 5
                    self.targetisleft = False
                    self.targetisright = True
    ...
    def updatecharacter(self):
        if self.health <= 0:
            ...
            self.image = self.image.load("slime.png")
            self.image.convert()
...
def death_screen():
    ...
    while dead:
        ...
        if mouse[0] in range(0, 240):
            button(104, 344, 152, 52, 1, 4)
            # print(focus)
        else:
            button(344, 344, 152, 52, 2, 4)
...
def next_level():
    ...
    pause = levelone = dead = diffsel = False
    gamerun = True
    leveltwo = True
    return leveltwo, gamerun

...
def main():
    ...
    enemya = Enemy(360, 64, False, True, player)
    enemyoverlay = Overlay(208, 0, enemy)
    lefto = False
    righto = False
    platforms = []
...
def text_object(text, font, colour):  # text function
...
def button(msg, x, y, width, height, colour1, colour2, func=False):  # button function
    ...
    # print(mouse_pos)
    pygame.draw.rect(screen, colour1, (x, y, width, height))  # render button
    if x + width > mouse_pos[0] > x and y + height > mouse_pos[1] > y:  # check if mouse is over button
        pygame.draw.rect(screen, colour2, (x, y, width, height))  # change button colour
        if mouse_click[0] == 1 and func != None:
            func()  # runs fuction
    else:
        pygame.draw.rect(screen, colour1, (x, y, width, height))  # resting colour of button
    smallText = pygame.font.Font("freesansbold.ttf", 40)  # creates a font
    text_surf, text_rect = text_object(msg, smallText, Color("#000000"))  # renders that font
    text_rect.center = ((x + (width / 2)), (y + (height / 2)))  # positions that font
    screen.blit(text_surf, text_rect)  # print button
